    ...
    <div className="max-w-4xl mx-auto px-6 py-12 md:py-20">
        ...
        {updates.map((update, idx) => (
          <div key={idx} className="relative pl-8 border-l border-[var(--border-subtle)]">
            ...
            <div className="mb-2 flex items-center gap-3">
              ...
              <span className="px-2 py-0.5 rounded bg-[var(--surface)] border border-[var(--border-subtle)] text-[10px] font-mono text-white">{update.version}</span>
            </div>
            ...
            <ul className="space-y-3">
              {update.changes.map((change, cIdx) => (
                <li key={cIdx} className="flex items-start gap-3 text-sm text-white/80">
      ...
      <div className="mt-20 glass-panel p-8 text-center bg-[rgba(123,97,255,0.05)] border-[var(--neon-purple-glow)]">
        ...
        <div className="flex max-w-md mx-auto gap-2">
          <input type="email" placeholder="email@example.com" className="flex-1 bg-[var(--surface)] border border-[var(--border-subtle)] rounded-lg px-4 py-2 text-white text-sm focus:outline-none focus:border-[var(--neon-purple)]" />
          <button className="btn-primary px-6 py-2 text-sm whitespace-nowrap" style={{ background: 'var(--neon-purple)', color: 'white' }}>Notify Me</button>
        </div>
      </div>
    </div>
  );
